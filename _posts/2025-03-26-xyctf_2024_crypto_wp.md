...
先把利用$(p+qi)*(p+qi)$解p,q,发现$phi=(p^2-1)*(q^2-1),不与$e互质,用简单一点都AMM算法解决
...
$seed_{i+1}=a*seed_i+b\ mod\ p$已知三十个seed的高八位

写成$seed_{i+1}^{'}+out_{i+1}=a*seed_i+a*out_{i}+b\ mod\ p$

令$b^{'}_i=b+a*out_i-out_{i+1}$则
$seed_{i+1}^{'}=a*seed_i+b^{'}_i\ mod\ p$

记第一个方程为$seed_{1}^{'}=a*seed_0+b^{'}_0\ mod\ p$
逐个带入有$seed_{i}^{'}=a_i*seed_0+b_i^{''}\ mod\ p$,  其中$a_i=a^{i+1}$,$b_i^{''}=\sum_{i=1}^{n} a_{i-1}i*b^{'}_i$
所以有格
\[M=
\left(
\begin{array}{ccc}
...
\]
有
$(k_0,k_1,...,k_{29},1,seed_{0}^{'})*M=(seed^{'}_{1},...,seed^{'}_{29},2^{119},seed^{'}_{0})$
用LLL算法即可得到
exp
```python
from Crypto.Util.number import *
